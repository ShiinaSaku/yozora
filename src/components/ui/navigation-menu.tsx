...
export const navigationMenuTriggerVariants = cva(
  "group/navigation-menu-trigger inline-flex h-9 w-max items-center justify-center transition-colors outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-1 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default:
  ...
  return (
    <NavigationMenuPrimitive.Portal>
      <NavigationMenuPrimitive.Positioner
        ...
        {...props}
      >
        <NavigationMenuPrimitive.Popup className="xs:w-(--popup-width) relative h-(--popup-height) w-(--popup-width) origin-(--transform-origin) rounded-2xl bg-popover text-popover-foreground shadow ring-1 ring-foreground/5 transition-all duration-300 ease-out outline-none data-ending-style:scale-90 data-ending-style:opacity-0 data-ending-style:duration-150 data-starting-style:scale-90 data-starting-style:opacity-0">
...
function NavigationMenuLink({
  ...
  return (
    <NavigationMenuPrimitive.Link
      ...
      className={cn(
        "flex items-center gap-1.5 rounded-2xl p-3 text-sm transition-colors outline-none hover:bg-muted focus:bg-muted focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-1 in-data-[slot=navigation-menu-content]:rounded-xl data-[active=true]:bg-muted/50 data-[active=true]:hover:bg-muted data-[active=true]:focus:bg-muted [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
...
function NavigationMenuIndicator({
  ...
  return (
    <NavigationMenuPrimitive.Icon
      ...
      {...props}
    >
      <div className="relative top-1/2 h-2 w-2 rotate-45 rounded-tl-sm bg-border shadow-md" />
    </NavigationMenuPrimitive.Icon>
  )
}
