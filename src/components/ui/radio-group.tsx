...
export function RadioGroup({
  ...
  return (
    <RadioGroupPrimitive
      render={<div className={cn("flex flex-col gap-3", className)} />}
...
export function Radio({
  ...
  return (
    <RadioPrimitive.Root
      className={cn(
        "relative inline-flex size-4.5 shrink-0 items-center justify-center rounded-full border border-input bg-background shadow-xs/5 transition-shadow outline-none not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-full focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background aria-invalid:border-destructive/36 focus-visible:aria-invalid:border-destructive/64 focus-visible:aria-invalid:ring-destructive/48 sm:size-4 dark:not-data-checked:bg-input/32 dark:aria-invalid:ring-destructive/24 data-disabled:cursor-not-allowed data-disabled:opacity-64 [[data-disabled],[data-checked],[aria-invalid]]:shadow-none",
        className
      )}
      data-slot="radio"
      {...props}
